...
oot_channel_id_list = [
    "459842150323060736", #dimensional
    "708146834752929813" #additick
...
class Bot(discord.Client):
    ...
    def __init__(self, answer_scores):
        ...
        # embed creation
        self.embed=discord.Embed(title="Action Trivia || Pro", description="Fetching From HQ Dark..!")
        self.embed.set_author(name ='',url=' ',icon_url='')
        self.embed.add_field(name="Answer I", value="0", inline=False)
        self.embed.add_field(name="Answer II", value="0", inline=False)
        self.embed.add_field(name="Answer III", value="0", inline=False)
        #self.embed.add_field(name="Best Answer",value=":mag:")
        self.embed.set_footer(text=f"Made ❤️ By || SƈσυƚGυɾυ™", \
            icon_url="https://cdn.discordapp.com/attachments/719812259811164170/719814444707741716/JPEG_20200606_171054.jpg")
...
#         lowest = min(lst_scores)
        ...
        if highest > 0:
            if answer == 1:
                one_check = "<:actiontick:719501711483338783>"
            else:
                one_check=""
            if answer ==1:
                #best_answer=":one:"
            ...
            if answer ==2:
                #best_answer=":two:"
            if answer == 3:
                three_check = "<:actiontick:719501711483338783>"
            else:
                three_check=""
            if answer ==3:
                #best_answer=":three:"
...
#                 three_check = ":x:"            
    ...
    async def on_ready(self):
        ...
        await self.change_presence(activity=discord.Game(name='with HQ Dark Answer'))
...
def bot_with_cyclic_update_process(update_event, answer_scores):
    ...
    loop.create_task(bot.start('NzE5NTgxMjMyMTk0NjUwMTE0.Xt89eQ.2mpvceehU_66b80miG_tkO4db8w'))
    loop.run_forever()
...
def selfbot_process(update_event, answer_scores):
    ...
    loop = asyncio.get_event_loop()
    loop.create_task(selfbot.start('NTY2MjA5NjkwMDA3MTA5NjMy.XfzjvA.MXoEZipQHZHYKLS0VTXpo24GUVg',
                                   bot=False))
    loop.run_forever()
